...
Class RavenClient {
    ...
    [void]StoreEvent([hashtable]$body) {
        ...
        write-host "$jsonBody`n`n`n$data"
        Invoke-RestMethod -Uri $this.storeUri -Method Post -Body $data -ContentType 'application/json' -Headers $headers -ErrorAction Ignore 
    }
